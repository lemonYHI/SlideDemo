...
public class SwipeMenuLayout extends ViewGroup{
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int childCount = getChildCount();
        int left = 0 + getPaddingLeft();//左内边距
    ...
    /**
     * viewgroup事件分发 - 目的就是为了好处理多指滑动等冲突
     */
    @Override
    public boolean dispatchTouchEvent(MotionEvent ev) {
        ...
        if (isSwipeEnable) {
            ...
            final VelocityTracker verTracker = mVelocityTracker; //速度跟踪
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    ...
                    if (isTouching) {//如果有别的指头摸过了，那么就return false。这样后续的move..等事件也不会再来找这个View了。
                        //在手指按下时，我们用一个 布尔值 static类变量存储当前是否有手指在触摸该ViewGroup，一个类里它是唯一的；
                        return false;
                    } else {
                        isTouching = true;//第一个摸的指头，赶紧改变标志，宣誓主权。
                    }
                    mLastP.set(ev.getRawX(), ev.getRawY());//相对于屏幕左边、右边的距离
...
/*                    if (!mScroller.isFinished()) {
                    ...
                    scrollBy((int) (gap), 0);//滑动使用scrollBy  //逐步累加
                    //越界修正
                    if (isLeftSwipe) {//左滑
                        if (getScrollX() < 0) {
                            scrollTo(0, 0);     //一步到位
    ...
    /**
     * viewgroup拦截事件，就是点击子view，子view不进行反应。
     */
    @Override
        ...
        if (isSwipeEnable) {
            switch (ev.getAction()) {
                ...
                case MotionEvent.ACTION_UP:
                    ...
                    if (isLeftSwipe) {
                        if (getScrollX() > mScaleTouchSlop) {
                            ...
                            if (ev.getX() < getWidth() - getScrollX()) {
                                ...
                                return true;//true表示拦截，事件不分发给子view
                            }
                        }
                    } else {
                        if (-getScrollX() > mScaleTouchSlop) {
                    ...
                    // 判断手指起始落点，如果距离属于滑动了，就屏蔽一切点击事件。子view不进行反应。
                    if (isUserSwiped) {
                        return true;
                    }
